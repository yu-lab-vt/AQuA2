function [z_score1,z_score2,t_score1,t_score2] = getSeedScore_DS3(pix,t_scl,datVec,sz)
    %% downsample
    ...
    for i = 1:numel(ihw)
        ...
        if ~isempty(bg1)
            L = (mean(fg) - mean(bg1));
            [mu,sigma] = se.ordStatApproxKsecWith0s(fg,bg1,bg2);
%             z_Left(i) = (L-mu)/sigma;
        ...
        if ~isempty(bg2)
            L = (mean(fg) - mean(bg2));
            [mu,sigma] = se.ordStatApproxKsecWith0s(fg,bg2,bg1);
