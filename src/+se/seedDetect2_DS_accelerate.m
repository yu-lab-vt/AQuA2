function [Map,arLst] = seedDetect2_DS_accelerate(dF,datOrg,arLst,opts,ff)
    ...
    for k = 1:numel(Thrs)   % threshold
        ...
        for j = 1:numel(scaleRatios)    % downsample rate
            ...
            for i = 1:numel(curRegions)
                ...
                [z_score1,z_score2,t_score1,t_score2] = se.getSeedScore_DS4(pix,t_scl,datResize{j},[H0,W0,L,T]);

                if min([z_score1,z_score2,t_score1,t_score2])>opts.sigThr
                    %% check seed curve significance
                    curve = se.myResize(mean(datResize{j}(ihw,:),1),1/t_scl);
                    hasPeak =  se.curveSignificance3(curve,max(1,floor(min(it)/t_scl)),ceil(max(it)/t_scl),opts.sigThr);
                    if hasPeak
%                         %% update
                        zscoreMap(pixOrg) = max(min(z_score1,z_score2),zscoreMap(pixOrg));
                    else
%                         figure;plot(curve);hold on;plot(min(it):max(it),curve(min(it):max(it)));
%                          keyboard;
                    end
                end
            end
    ...
    sdLst = bwconncomp(zscoreMap>0);
    sdLst = sdLst.PixelIdxList;
    Map = zeros(size(zscoreMap),'uint16');
    cnt = 0;
    for i = 1:numel(sdLst)
        pix = sdLst{i};
        scores = unique(zscoreMap(pix));
        if numel(scores)>1
            pix = pix(zscoreMap(pix) == max(scores));
            cnt = cnt + 1;
        end
        Map(pix) = i;
    end
