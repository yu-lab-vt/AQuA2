...
BW = true(size(Map0));
BW(pix0) = false;
BW2 = imerode(BW,strel('cube',8));
scoreMap0(BW) =  max(scoreMap0(:));
scoreMap0(BW2) = 0;
...
%                 labelWaterShed = setdiff(MapOut,0);
% parallel
subevents = cell(numel(sdLst),1);
for ii = 1:numel(sdLst)
   target = sdLst{ii};
   waterLabels = MapOut(target(1));
   subevents{ii} = [waterLst{waterLabels}];
end
labelMap = zeros(H0,W0,T0);
for i = 1:numel(subevents)
    labelMap(subevents{i}) = i;
end

[x_dir,y_dir,z_dir] = se.dirGenerate(26);
AllSelectPix = find(signal>0.08);
AllSelectPix = AllSelectPix(labelMap(AllSelectPix)==0);

[ih0,iw0,it0] = ind2sub([H0,W0,T0],AllSelectPix);
for k = 1:numel(x_dir)
    ih = max(1,min(H0,ih0-x_dir(k)));
    iw = max(1,min(W0,iw0-y_dir(k)));
    it = max(1,min(T0,it0-z_dir(k)));
    pixCur = sub2ind([H0,W0,T0],ih,iw,it);
    select = labelMap(pixCur)>0;
    labelMap(AllSelectPix(select)) = labelMap(pixCur(select));
    ih0 = ih0(~select);
    iw0 = iw0(~select);
    it0 = it0(~select);
    AllSelectPix = AllSelectPix(~select);
end

subevents = label2idx(labelMap);
labelMap(labelMap==3) = 1;


%%
signal(labelMap==4)=0;
signal = reshape(signal,[],T0);
pix = find(sum(labelMap,3)>0);
delayMap = zeros(H0,W0);
thrs = 0.3:0.1:0.7;
for i = 1:numel(pix)
   curve = signal(pix(i),:);
   delays = zeros(numel(thrs),1);
   for k = 1:numel(thrs)
       delays(k) = find(curve>thrs(k)*max(curve),1);
   end
   delayMap(pix(i)) = mean(delays);
end
cmap = parula(256);
% Make lowest one black
cmap(1,:) = 1;
figure('Position',[100,100,300,400]);
imagesc(delayMap);colormap(cmap);colorbar;
axis off;
...
for ii=1:T0
    ...
    img0(labelMap(:,:,ii)>0) = img0(labelMap(:,:,ii)>0)*0.4;
    imgx = cat(3,img0,img0,img0)+Lx;
    alphaMap = ones(H0,W0)*0.2 + (labelMap(:,:,ii)>0)*0.5;
    %%
